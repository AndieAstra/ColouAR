...
@Component({
  selector: 'app-pizza-ar',
  standalone: true,
  imports: [],
  templateUrl: './pizza-ar.component.html',
  styleUrl: './pizza-ar.component.scss',
  schemas: [CUSTOM_ELEMENTS_SCHEMA]
})
export class PizzaArComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    // Render button
    nativeElement.querySelector('#render-button')?.addEventListener('click', () => {
      this.onOpenCvReady(); // Existing functionality
      this.loadSecondModel(); // Load the second model
    });
    ...
    if (example3D) {
     ...
     // Handle second model load
    const pizzaBoxModel = document.querySelector('#pizza-box-model') as any;
    if (pizzaBoxModel) {
      pizzaBoxModel.addEventListener('model-loaded', () => {
  ...
  onOpenCvReady() {
    ...
    if (example3D && example3D.object3D) {
      ...
      // Set the model back to pizza.glb from colored model
      example3D.setAttribute('gltf-model', 'assets/models/pizza.glb');
      example3D.setAttribute('scale', '0.5 0.5 0.5');
    }
  ...
  loadNewModel() {
    ...
    if (example3D) {
      // Load pre-colored pizza model
      example3D.setAttribute('gltf-model', 'assets/models/color-pizza.glb');
      example3D.setAttribute('scale', '0.5 0.5 0.5');

      // Hide pizza-box model to match
      if (pizzaBoxModel) {
        pizzaBoxModel.setAttribute('visible', 'false'); // Or remove it if needed
        console.log('Pizza box model is now hidden.');
      }
    }
  }
  ...
  loadSecondModel() {
    const pizzaBoxModel = document.querySelector('#pizza-box-model') as any;
    if (pizzaBoxModel) {
      console.log('Attempting to load pizza-box model...');

      // Set attributes for the pizza-box model
      pizzaBoxModel.setAttribute('gltf-model', 'assets/models/pizza-box.glb');
      pizzaBoxModel.setAttribute('scale', '0.5 0.5 0.5');
      pizzaBoxModel.setAttribute('position', '0 -0.5 0');
      pizzaBoxModel.setAttribute('visible', 'true');

      // Add a model-loaded listener for debugging
      pizzaBoxModel.addEventListener('model-loaded', () => {
        console.log('Pizza-box model successfully loaded and visible.');
      });
    } else {
      console.error('pizza-box-model entity not found in the DOM!');
    }
  }
